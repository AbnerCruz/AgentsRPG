...
newBtn.onclick=async()=>{
 ...
 try{
  ...
  await new Promise(requestAnimationFrame);
  await boot(createNewSimulation(seed,generated),0);
 }catch(error){
  console.error(error);
...
async function generateWorld(seed){
 ...
 return await new Promise(resolve=>{let settled=false;const worker=new Worker(new URL('./world/world-worker.js',import.meta.url),{type:'module'}),finish=value=>{if(settled)return;settled=true;clearTimeout(timer);worker.terminate();resolve(value)},timer=setTimeout(()=>finish(null),45000);worker.onmessage=e=>{const msg=e.data||{};if(msg.type==='progress'){status.textContent=`Gerando mundo · ${msg.stage} · ${Math.round((msg.value||0)*100)}%`;return}if(msg.type==='done')finish(msg.data);else if(msg.type==='error'){console.error('world worker',msg.message);finish(null)}};worker.onerror=e=>{console.error('world worker',e);finish(null)};worker.postMessage({seed})})
}
async function boot(instance,catchupTicks=0){sim=instance;if(catchupTicks>0)await catchUp(catchupTicks);document.querySelector('#boot').classList.add('hidden');renderer=new Renderer(document.querySelector('#world'),sim,i=>ui.inspect(i));installLayerInspector(renderer,document.querySelector('#layerSelect'));installPhaseCOverlay(renderer);loop=new GameLoop(()=>{sim.step();if(sim.tick-lastSaveTick>=DAY_TICKS){lastSaveTick=sim.tick;scheduleSave()}},alpha=>{renderer.render(alpha);ui.update()});ui=new UI(sim,renderer,loop);installPhaseBUI(ui);loop.start();scheduleSave(true);addEventListener('agents-save',()=>scheduleSave(true));document.addEventListener('visibilitychange',()=>{if(document.hidden)scheduleSave(true)});window.addEventListener('pagehide',()=>saveWorld(sim.serialize()));ui.update();ui.toast?.(`Mundo ${sim.seed} · geração ${sim.world.generationStats?.ms??0} ms`)}
