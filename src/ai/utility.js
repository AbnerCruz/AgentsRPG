...
export function scoreActions(sim,i,perception={}){
 ...
 const scores=[
  ...
  [ACTION.WOOD,(.08+woodDemand*.82)*distCost(wood?.d)*(wood?.confidence??0)*(.5+n.skill(i,1))*spec(2)],
  ...
  [ACTION.FISH,fish?(.1+(ownFood<.8?.35:.03))*distCost(fish.d)*(.45+n.skill(i,4))*fishStage*spec(7):0],
  ...
  [ACTION.EXPLORE,(.05+(1-coverage)*.3+ignorance+need(NEED.PURPOSE)*.12)*(.2+g(GENE.CURIOSITY)*g(GENE.CURIOSITY)*.9)*(1-g(GENE.CAUTION)*.28)*(1-need(NEED.HUNGER)*.72)*(1-need(NEED.THIRST)*.74)*(1-Math.min(.7,injury*.45))*(night?.38:1)],
