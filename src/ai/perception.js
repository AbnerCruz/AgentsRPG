...
export function perceive(sim,i){
 const n=sim.npcs,w=sim.world,m=sim.memory,d=n.derived(i),night=isNight(sim.tick),biome=w.biome(n.x[i],n.y[i]),tech=technologyFor(sim);
 let short=d.perception*BALANCE.perceptionShortMult,long=d.longPerception*BALANCE.perceptionLongMult;
 if(night){short*=.42;long*=.35;const fire=w.nearestBuildingLocal?.(n.x[i],n.y[i],BUILDING.CAMPFIRE,5);if(fire?.ref?.lit){short*=1.7;long*=1.25}}
 ...
 const buildings=[];for(const b of w.buildings){if(!b.finished)continue;const bd=Math.hypot(b.x-n.x[i],b.y-n.y[i]);if(bd<=short){buildings.push({id:b.id,type:b.type,x:b.x,y:b.y,d:bd,owner:b.owner,lit:b.lit,fuel:b.fuel||0});m.observe(i,w.idx(b.x,b.y),sim.tick,0,SPATIAL_FLAG.BUILDING|(b.type===BUILDING.CAMPFIRE?SPATIAL_FLAG.FIRE:0),cap,1,b.owner??-1)}else if(b.type===BUILDING.CAMPFIRE&&b.lit&&bd<=Math.max(long,night?44:long)&&lineOfSight(w,n.x[i],n.y[i],b.x,b.y,.08)){m.observe(i,w.idx(b.x,b.y),sim.tick,0,SPATIAL_FLAG.SMOKE|SPATIAL_FLAG.FIRE,cap,1,b.owner??-1)}}
 ...
 const prey=[],huntingTech=tech.knows(n,i,13);for(const a of sim.animals?.items||[])if(a.alive){const ad=Math.hypot(a.x-n.x[i],a.y-n.y[i]);if(ad>short)continue;const info=ANIMAL_INFO[a.species]||{};if(a.predator){const t=(1-ad/(short+1))*(.12+a.power*.38);if(t>threat){threat=t;target={kind:'animal',ref:{id:a.id},x:a.x,y:a.y,d:ad}}}else if(!a.domestic&&!a.captive){if(!huntingTech&&!info.small)continue;prey.push({id:a.id,species:a.species,x:a.x,y:a.y,d:ad,risk:info.power||0,small:!!info.small,meat:info.meat||.2})}}
 prey.sort((a,b)=>(a.risk+n.skill(i,3)*-.35+a.d*.02)-(b.risk+n.skill(i,3)*-.35+b.d*.02));
