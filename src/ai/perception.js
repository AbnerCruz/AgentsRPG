...
export function perceive(sim,i){
 ...
 const buildings=[];for(const b of w.buildings){if(!b.finished)continue;const bd=Math.hypot(b.x-n.x[i],b.y-n.y[i]);if(bd<=short){buildings.push({...b,d:bd});m.observe(i,w.idx(b.x,b.y),sim.tick,0,SPATIAL_FLAG.BUILDING|(b.type===BUILDING.CAMPFIRE?SPATIAL_FLAG.FIRE:0),cap,1,b.owner??-1)}else if(b.type===BUILDING.CAMPFIRE&&bd<=Math.max(long,night?44:long)&&lineOfSight(w,n.x[i],n.y[i],b.x,b.y,.08)){m.observe(i,w.idx(b.x,b.y),sim.tick,0,SPATIAL_FLAG.SMOKE|SPATIAL_FLAG.FIRE,cap,1,b.owner??-1)}}
 const dungeons=[];for(const dg of w.dungeons){const dd=Math.hypot(dg.x-n.x[i],dg.y-n.y[i]);if(dd<=long&&lineOfSight(w,n.x[i],n.y[i],dg.x,dg.y,.06)){dungeons.push({...dg,d:dd});m.observe(i,w.idx(dg.x,dg.y),sim.tick,0,SPATIAL_FLAG.DUNGEON,cap,1,-1);const key=`dungeon:${dg.id}`;if(!(m.semantic.get(i)||[]).some(f=>f.key===key)){m.learn(i,{key,text:`Há uma entrada de dungeon nesta região.`,tick:sim.tick,confidence:.9});m.remember(i,{type:'descoberta',text:'Encontrei uma entrada de dungeon desconhecida.',tick:sim.tick,valence:1,importance:.82,reflectionKey:'exploração'});sim.log('descoberta',`${n.names[i]} descobriu uma dungeon.`,.78,i)}}}
 ...
 let threat=0,target=null;for(const mon of w.monsters)if(mon.hp>0){const md=Math.hypot(mon.x-n.x[i],mon.y-n.y[i]);if(md<=short){const t=(1-md/(short+1))*(.45+mon.power);if(t>threat){threat=t;target={kind:'monster',ref:mon,x:mon.x,y:mon.y,d:md}}m.observe(i,w.idx(mon.x,mon.y),sim.tick,0,SPATIAL_FLAG.THREAT,cap,1,-1)}}
 const prey=[];for(const a of sim.animals?.items||[])if(a.alive){const ad=Math.hypot(a.x-n.x[i],a.y-n.y[i]);if(ad>short)continue;if(a.predator){const t=(1-ad/(short+1))*(.12+a.power*.38);if(t>threat){threat=t;target={kind:'animal',ref:a,x:a.x,y:a.y,d:ad}}}else if(!a.domestic)prey.push({...a,d:ad})}
 const senses=sensoryFor(sim);senses.tick(sim.tick);const sounds=senses.hear(sim,i),smells=senses.smell(sim,i);for(const s of sounds)if(['grito','combate','uivo'].includes(s.type)&&s.confidence>.25)threat=Math.max(threat,.15+s.confidence*.25);
 return{radius:short,longRadius:long,threat:Math.min(1.5,threat),target,resources:seenResources,humans,wounded,prey,buildings,dungeons,sounds,smells};
}
