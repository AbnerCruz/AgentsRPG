...
if(!AnimalSystem.prototype.__phaseERefugeInstalled){
 ...
 AnimalSystem.prototype.regionalTick=function(sim){
  ...
  for(let ry=0;ry<12;ry++)for(let rx=0;rx<12;rx++)for(let s=0;s<ANIMAL.NAMES.length;s++){
   ...
   if(this.liveCount()>=MAX_ANIMALS&&!makeRoomForRefuge(this,s))continue;
...
function makeRoomForRefuge(system,protectedSpecies){
 system.recountRegional();let victim=-1,best=-Infinity;
 for(let i=0;i<system.count;i++){
  if(!system.alive[i]||system.domestic[i]||system.captive[i]||system.species[i]===protectedSpecies)continue;
  const s=system.species[i],rx=Math.floor(system.x[i]/16),ry=Math.floor(system.y[i]/16),idx=system.regionSpeciesIndex(rx,ry,s),K=system.capacityFor(s,rx,ry,system.world,'primavera'),excess=system.regionPop[idx]-Math.max(2,K);
  if(excess>best){best=excess;victim=i}
 }
 if(victim<0)return false;
 system.killIndex(victim,'dispersão ecológica');system.recountRegional();return true;
}
