import {ACTION,GENE,RESOURCE,TECH_COUNT,TOOL} from '../core/constants.js';
...
export class TechnologySystem{
 ...
 canAction(i,action,sim){const t=ACTION_TECH.get(action);return t==null||this.knows(sim.npcs,i,t)}
 countKnown(n,i){let c=0;for(let t=0;t<TECH_COUNT;t++)if(this.knows(n,i,t))c++;return c}
 prerequisites(n,i,t){return TECH[t].pre.every(p=>this.knows(n,i,p))}
 availableMaterial(sim,i,t){const inv=sim.npcs.inventory[i],need=TECH[t].mat;if(!need.length)return true;return need.some(k=>inv[k]>.04||sim.memory.recallNearest(i,sim.npcs.x[i],sim.npcs.y[i],k,sim.tick,840*30))}
 progress(n,i,t,amount,sourceUid=-1,tick=0){if(this.knows(n,i,t)||!this.prerequisites(n,i,t))return false;const p=i*TECH_COUNT+t;n.techProgress[p]=Math.min(255,n.techProgress[p]+Math.max(1,amount|0));if(sourceUid>=0)n.techSource[p]=sourceUid;if(n.techProgress[p]>=255)return this.setKnown(n,i,t,sourceUid,tick);return false}
 experiment(sim,i){const n=sim.npcs;if(n.age[i]<12||n.need(i,0)>.78||n.need(i,1)>.78)return null;const candidates=[];for(let t=0;t<TECH_COUNT;t++)if(!this.knows(n,i,t)&&this.prerequisites(n,i,t)&&this.availableMaterial(sim,i,t))candidates.push(t);if(!candidates.length)return null;const creativity=n.gene(i,GENE.CREATIVITY),curiosity=n.gene(i,GENE.CURIOSITY),frustration=Math.min(1,(n.need(i,6)||0)+.15);const chance=.018*(.25+creativity*.75)*(.35+curiosity*.65)*(.55+frustration*.45);if(!sim.rng.chance(chance))return null;const t=sim.rng.pick(candidates),gain=28+Math.round((creativity+curiosity)*42+sim.rng.range(0,35));const learned=this.progress(n,i,t,gain,-1,sim.tick);if(learned)this.onDiscovery(sim,i,t,'experimentação');return{tech:t,learned}}
 observe(sim,observer,actor,action){const t=ACTION_TECH.get(action);if(t==null)return false;const n=sim.npcs;if(!this.knows(n,actor,t)||this.knows(n,observer,t)||!this.prerequisites(n,observer,t))return false;const hunger=n.need(observer,0),attention=Math.max(.1,1-hunger*.8),gain=Math.round((8+n.gene(observer,GENE.LEARNING)*18)*attention);const learned=this.progress(n,observer,t,gain,n.uid[actor],sim.tick);if(learned)this.onDiscovery(sim,observer,t,`observando ${n.names[actor]}`);return learned}
 teach(sim,teacher,student){const n=sim.npcs;if(teacher<0||student<0||teacher===student)return null;const options=[];for(let t=0;t<TECH_COUNT;t++)if(this.knows(n,teacher,t)&&!this.knows(n,student,t)&&this.prerequisites(n,student,t))options.push(t);if(!options.length)return null;const t=sim.rng.pick(options),rel=sim.memory.relation(teacher,student),kin=n.parents[student]?.includes(n.uid[teacher])||n.parents[teacher]?.includes(n.uid[student]);const willingness=.25+n.gene(teacher,GENE.LOYALTY)*.35+(rel.affection+.5)*.22+(kin?.22:0)-n.gene(teacher,GENE.GREED)*.28;if(!sim.rng.chance(Math.max(.04,willingness))){sim.memory.remember(student,{type:'recusa',text:`${n.names[teacher]} recusou ensinar ${TECH[t].name}.`,tick:sim.tick,valence:-2,importance:.45,reflectionKey:`pessoa:${teacher}`});return{tech:t,learned:false,refused:true}}const gain=55+Math.round(n.gene(student,GENE.LEARNING)*65);const learned=this.progress(n,student,t,gain,n.uid[teacher],sim.tick);sim.memory.adjust(student,teacher,{trust:.035,affection:.018,debt:.04,respect:.025});sim.memory.adjust(teacher,student,{affection:.012,respect:.008});if(learned)this.onDiscovery(sim,student,t,`ensinado por ${n.names[teacher]}`);return{tech:t,learned,refused:false}}
 inherit(sim,parent,child){const n=sim.npcs;if(child<0||parent<0)return;for(let t=0;t<TECH_COUNT;t++)if(this.knows(n,parent,t)&&this.prerequisites(n,child,t)&&!this.knows(n,child,t)){const gain=2+Math.round(n.gene(child,GENE.LEARNING)*4);const learned=this.progress(n,child,t,gain,n.uid[parent],sim.tick);if(learned)this.onDiscovery(sim,child,t,`aprendido na família com ${n.names[parent]}`)}}
 onDiscovery(sim,i,t,channel){const n=sim.npcs,tech=TECH[t];this.discoveries.push({tick:sim.tick,npc:n.uid[i],tech:t,channel});if(this.discoveries.length>600)this.discoveries.shift();sim.memory.remember(i,{type:'técnica',text:`Aprendi ${tech.name} por ${channel}.`,tick:sim.tick,valence:5,importance:.9,reflectionKey:`tecnica:${tech.key}`});sim.log('descoberta',`${n.names[i]} aprendeu ${tech.name}.`,.92,i);if(t===4&&n.tool[i]===TOOL.NONE)n.tool[i]=TOOL.AXE;if(t===5&&n.tool[i]===TOOL.NONE)n.tool[i]=TOOL.PICK;if(t===6&&n.tool[i]===TOOL.NONE)n.tool[i]=TOOL.KNIFE}
}

export function technologyFor(sim){if(!sim.technology)sim.technology=new TechnologySystem();return sim.technology}
export function techniqueForAction(action){return ACTION_TECH.get(action)??null}
