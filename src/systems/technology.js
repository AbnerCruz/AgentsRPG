...
export const TECH=[
 ...
 {id:36,key:'sun_drying',name:'Secar alimento ao sol',pre:[9],mat:[RESOURCE.MEAT,RESOURCE.FISH]},
 {id:37,key:'smoking',name:'Defumar alimento',pre:[3,9],mat:[RESOURCE.LOG,RESOURCE.MEAT]},
 {id:38,key:'salting',name:'Salgar alimento',pre:[9],mat:[RESOURCE.SALT,RESOURCE.MEAT]},
 {id:39,key:'fermentation',name:'Fermentar alimento',pre:[19],mat:[RESOURCE.MILK,RESOURCE.BERRY]},
 {id:40,key:'cellar',name:'Adega subterrânea',pre:[22,19],mat:[RESOURCE.STONE,RESOURCE.CLAY]},
 {id:41,key:'waterskin',name:'Odre de couro',pre:[10,11],mat:[RESOURCE.LEATHER]},
 {id:42,key:'water_vessel',name:'Recipiente cerâmico',pre:[19],mat:[RESOURCE.CLAY]}
];
...
export class TechnologySystem{
 constructor(sim=null,data=null){this.sim=sim;this.discoveries=data?.discoveries||[];this.crossGroupTransfers=data?.crossGroupTransfers||0;this.focusByUid=new Map(data?.focusByUid||[]);this.lastAttemptByUid=new Map(data?.lastAttemptByUid||[])}
 ...
 canAction(i,action,sim=this.sim){if(action===ACTION.WOOD||action===ACTION.FISH||action===ACTION.HUNT)return true;const t=ACTION_TECH.get(action);if(t==null)return true;const n=sim?.npcs;if(!n)return false;return this.knows(n,i,t)}
 ...
 availableMaterial(sim,i,t){const n=sim.npcs,w=sim.world,inv=n.inventory[i],need=TECH[t].mat;if(!need.length)return true;const x=n.x[i],y=n.y[i],tile=w.tile(x,y),biome=w.biome(x,y);return need.some(k=>{if(inv[k]>.04)return true;if(sim.memory.recallNearest(i,x,y,k,sim.tick,840*30))return true;if(w.resourcesNear?.(x,y,2.5,k)?.length)return true;if(k===RESOURCE.STONE&&(tile===TILE_TYPE.STONE||biome===BIOME.ROCK||biome===BIOME.MOUNTAIN))return true;if(k===RESOURCE.LOG&&(tile===TILE_TYPE.FOREST||[BIOME.OPEN_FOREST,BIOME.DENSE_FOREST,BIOME.BOREAL].includes(biome)))return true;if(k===RESOURCE.WATER&&tile===TILE_TYPE.WATER)return true;return false})}
 ...
 experiment(sim,i){this.attach(sim);const n=sim.npcs,uid=n.uid[i];if(!uid||n.age[i]<12||n.need(i,0)>.84||n.need(i,1)>.84)return null;const last=this.lastAttemptByUid.get(uid)??-9999;if(sim.tick-last<BALANCE.experimentInterval)return null;this.lastAttemptByUid.set(uid,sim.tick);const candidates=[];for(let t=0;t<TECH_COUNT;t++)if(!this.knows(n,i,t)&&this.prerequisites(n,i,t)&&this.availableMaterial(sim,i,t))candidates.push(t);if(!candidates.length){this.focusByUid.delete(uid);return null}let t=this.focusByUid.get(uid);if(!candidates.includes(t)){t=sim.rng.weighted(candidates,x=>1+(n.techProgress[i*TECH_COUNT+x]||0)/80+(x<3?.35:0));this.focusByUid.set(uid,t)}const creativity=n.gene(i,GENE.CREATIVITY),curiosity=n.gene(i,GENE.CURIOSITY),stubborn=n.gene(i,GENE.STUBBORN),frustration=Math.min(1,(n.need(i,6)||0)+.15),p=n.techProgress[i*TECH_COUNT+t]||0,chance=.03*BALANCE.experimentChanceMult*(.25+creativity*.75)*(.35+curiosity*.65)*(.55+frustration*.45)*(1+p/255*.3)*(1+stubborn*.18);if(!sim.rng.chance(chance))return{tech:t,learned:false,progress:p};const gain=(62+Math.round((creativity+curiosity)*32+sim.rng.range(0,28)))*BALANCE.experimentGainMult,learned=this.progress(n,i,t,gain,-1,sim.tick);if(learned){this.focusByUid.delete(uid);this.onDiscovery(sim,i,t,'experimentação')}return{tech:t,learned,progress:n.techProgress[i*TECH_COUNT+t]}}
