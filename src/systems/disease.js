import {GENE,BIOME,RESOURCE,ACTION} from '../core/constants.js';
...
function proximitySpread(sim){const n=sim.npcs,alive=n.living();for(let a=0;a<alive.length;a++){const i=alive[a],active=n.activeDisease[i];if(!active)continue;const d=DISEASES[active-1];if(!d.infectivity||n.diseaseTimer[i]<d.incubation)return;for(let b=a+1;b<alive.length;b++){const j=alive[b];if(Math.hypot(n.x[i]-n.x[j],n.y[i]-n.y[j])>2.2)continue;if(sim.rng.chance(d.infectivity*.08)){infect(sim,j,d.id,i);break}}}}
