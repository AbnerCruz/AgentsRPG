...
export function foodExposure(sim,i,kind,raw=true){const t=technologyFor(sim),n=sim.npcs;let risk=(kind===RESOURCE.MEAT||kind===RESOURCE.FISH||kind===RESOURCE.MILK)?.035:(kind===RESOURCE.BERRY?.012:.004);if(t.knows(n,i,9)&&!raw)risk*=.12;if(kind===RESOURCE.BERRY&&t.knows(n,i,2))risk*=.22;if(t.knows(n,i,21))risk*=.55;if(sim.rng.chance(risk*BALANCE.diseaseSpreadMult))infect(sim,i,0)}
export function waterExposure(sim,i){const t=technologyFor(sim),n=sim.npcs,risk=t.knows(n,i,14)?.0015:.012;if(sim.rng.chance(risk*BALANCE.diseaseSpreadMult))infect(sim,i,0)}
export function diseaseTick(sim){const n=sim.npcs,t=technologyFor(sim);for(const i of n.living()){const active=n.activeDisease[i];if(active){const d=DISEASES[active-1];n.diseaseTimer[i]++;if(n.diseaseTimer[i]>=d.incubation){n.stamina[i]=Math.max(0,n.stamina[i]-d.severity*.00055);n.pain[i]=Math.min(1,n.pain[i]+d.severity*.0007);if(sim.rng.chance(d.lethality*BALANCE.diseaseLethalityMult*(1-n.gene(i,GENE.DISEASE)*.55)))n.hp[i]-=.02+d.severity*.025;if(n.diseaseTimer[i]>d.incubation+d.duration){if(d.immune)n.immunity[i]|=1<<d.id;n.activeDisease[i]=0;n.diseaseTimer[i]=0;n.pain[i]=Math.max(0,n.pain[i]-.2);sim.memory.remember(i,{type:'recuperação',text:`Me recuperei de ${d.name}.`,tick:sim.tick,valence:3,importance:.65,reflectionKey:'doença'})}}}
  if(sim.tick%60===i%60){const b=sim.world.biome(n.x[i],n.y[i]);if(b===BIOME.MARSH&&sim.rng.chance(.014*BALANCE.diseaseSpreadMult*(1-n.gene(i,GENE.DISEASE)*.6)))infect(sim,i,3);if(n.wound[i]>.28&&!t.knows(n,i,15)&&sim.rng.chance(.018*BALANCE.diseaseSpreadMult*(1-n.gene(i,GENE.DISEASE)*.6)))infect(sim,i,2);const cold=n.need(i,3);if(cold>.72&&sim.world.moisture?.[sim.world.idx(n.x[i],n.y[i])]>.55&&sim.rng.chance(.012*BALANCE.diseaseSpreadMult))infect(sim,i,1)}
 }
