...
import {diseaseTick,DISEASES} from './disease.js';
import {technologyFor,TECH} from './technology.js';
import {sensoryFor} from './senses.js';
export function lifecycleTick(sim){
 const n=sim.npcs,tech=technologyFor(sim),senses=sensoryFor(sim);diseaseTick(sim);
 for(const i of n.living()){
  ...
  if(sim.tick%84===i%84&&[ACTION.IDLE,ACTION.EXPLORE,ACTION.SOCIAL].includes(n.action[i]))tech.experiment(sim,i);
