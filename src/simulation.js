...
import {ensureTaskPlanning,performAtomicNeeds,performAtomicAction,handleEmergency,emergencyKind,resumeSuspended,shouldSwitchTask,switchTask,serializeTaskPlanning,hydrateTaskPlanning} from './systems/task-planning.js';
...
export class Simulation{
 ...
 reviewTask(i,p){const n=this.npcs,int=n.intent[i];if(!int)return;int.lastReview=this.tick;const scores=scoreActions(this,i,p||{});n.scores[i]=scores.slice(0,5);let goal=chooseAction(this,i,scores);if(p?.threat>.5){const flee=scores.find(x=>x[0]===ACTION.FLEE)?.[1]||0,fight=scores.find(x=>x[0]===ACTION.FIGHT)?.[1]||0;goal=flee>fight?ACTION.FLEE:ACTION.FIGHT}if(goal===int.action||!shouldSwitchTask(this,i,scores,goal,p))return;switchTask(this,i,goal,p);n.plan[i]=planFor(this,i,goal);if(goal===ACTION.EAT||goal===ACTION.DRINK){performAtomicAction(this,i,goal);resumeSuspended(this,i);return}if(!prepareAction(this,i,goal,p||{})&&goal!==ACTION.EXPLORE){if(!resumeSuspended(this,i))prepareAction(this,i,ACTION.EXPLORE,p||{})}}
 decide(i,p){const n=this.npcs,scores=scoreActions(this,i,p||{});n.scores[i]=scores.slice(0,5);let goal=chooseAction(this,i,scores);if(p?.threat>.5){const flee=scores.find(x=>x[0]===ACTION.FLEE)?.[1]||0,fight=scores.find(x=>x[0]===ACTION.FIGHT)?.[1]||0;goal=flee>fight?ACTION.FLEE:ACTION.FIGHT}n.plan[i]=planFor(this,i,goal);if(goal===ACTION.EAT||goal===ACTION.DRINK){performAtomicAction(this,i,goal);return}if(!prepareAction(this,i,goal,p||{})&&goal!==ACTION.EXPLORE)prepareAction(this,i,ACTION.EXPLORE,p||{})}
