...
export class Simulation{
 constructor(seed=1,generated=null){
  ...
  this.lastPerception=Array(MAX_NPCS).fill(null);this.initPopulation();const c=populationCenter(this);this.viewX=c.x;this.viewY=c.y;
 }
 ...
 step(){
  ...
  for(const i of n.living()){
   ...
   if(!n.intent[i]&&this.tick%45===i%45)this.technology.experiment(this,i);
 ...
 shouldInterrupt(i,p){const n=this.npcs,int=n.intent[i];if(!int)return false;const thirst=n.need(i,1),hunger=n.need(i,0),survival=[ACTION.WATER,ACTION.FORAGE,ACTION.EAT,ACTION.DRINK].includes(int.action);if(!survival&&thirst>.88&&![ACTION.DRINK,ACTION.WATER].includes(int.action))return true;if(!survival&&hunger>.91&&![ACTION.EAT,ACTION.FORAGE].includes(int.action))return true;if(!p?.target||int.action===ACTION.FIGHT||int.action===ACTION.FLEE)return false;const committed=[ACTION.EAT,ACTION.DRINK,ACTION.WATER,ACTION.FORAGE,ACTION.SLEEP].includes(int.action),threshold=(committed?.92:.48)+n.gene(i,GENE.STUBBORN)*.18-n.gene(i,GENE.CAUTION)*.12;return p.threat>threshold}
 reviewTask(i,p){const n=this.npcs,int=n.intent[i];if(!int)return;int.lastReview=this.tick;const scores=scoreActions(this,i,p||{}),current=scores.find(x=>x[0]===int.action);if(current){current[1]*=1+n.gene(i,GENE.STUBBORN)*.55+n.taskProgress[i]*.9;scores.sort((a,b)=>b[1]-a[1])}n.scores[i]=scores.slice(0,5);let goal=chooseAction(this,i,scores);if(p?.threat>.5){const flee=scores.find(x=>x[0]===ACTION.FLEE)?.[1]||0,fight=scores.find(x=>x[0]===ACTION.FIGHT)?.[1]||0;goal=flee>fight?ACTION.FLEE:ACTION.FIGHT}if(goal===int.action)return;abortIntent(this,i,'reavaliação de tarefa longa');n.plan[i]=planFor(this,i,goal);if(!prepareAction(this,i,goal,p||{})&&goal!==ACTION.EXPLORE)prepareAction(this,i,ACTION.EXPLORE,p||{})}
 decide(i,p){const n=this.npcs,scores=scoreActions(this,i,p||{});n.scores[i]=scores.slice(0,5);let goal=chooseAction(this,i,scores);if(p?.threat>.5){const flee=scores.find(x=>x[0]===ACTION.FLEE)?.[1]||0,fight=scores.find(x=>x[0]===ACTION.FIGHT)?.[1]||0;goal=flee>fight?ACTION.FLEE:ACTION.FIGHT}n.plan[i]=planFor(this,i,goal);if(!prepareAction(this,i,goal,p||{})&&goal!==ACTION.EXPLORE)prepareAction(this,i,ACTION.EXPLORE,p||{})}
 ...
 serialize(){return{version:VERSION,savedAt:Date.now(),seed:this.seed,rngState:this.rng.state,tick:this.tick,world:this.world.serialize(),npcs:this.npcs.serialize(),memory:this.memory.serialize(),technology:this.technology.serialize(),animals:this.animals.serialize(),dungeon:this.dungeon.serialize(),chronicle:this.chronicle,populationHistory:this.populationHistory,geneHistory:this.geneHistory,actionHistogram:this.actionHistogram,travel:this.travel,tasks:this.tasks}}
 static hydrate(d){
  if(!d||!d.seed)return new Simulation(1);const s=Object.create(Simulation.prototype);s.version=VERSION;s.seed=d.seed>>>0;s.rng=new RNG(s.seed);s.rng.state=d.rngState??s.rng.state;s.tick=d.tick||0;s.world=World.hydrate(d.world||{},s.rng,s.seed);s.npcs=NPCStore.hydrate(d.npcs||{});s.memory=MemorySystem.hydrate(d.memory);for(const i of s.npcs.living())s.memory.ensure(i,s.memory.spatialCapacity(s.npcs,i));s.technology=TechnologySystem.hydrate(d.technology,s);s.senses=new SensorySystem();s.animals=AnimalSystem.hydrate(d.animals,s.rng,s.world);if(!d.animals)s.animals=new AnimalSystem(s.rng,s.world);s.dungeon=Dungeon.hydrate(d.dungeon,s.rng,s.world);
  for(const i of s.npcs.living()){const int=s.npcs.intent[i];if(!int)continue;if(!s.npcs.taskDuration[i])s.npcs.taskDuration[i]=taskDurationFor(s,i,int.action,int);if(!s.npcs.taskTimeout[i])s.npcs.taskTimeout[i]=s.tick+Math.max(180,Math.ceil(s.npcs.taskDuration[i]*3)+240);if(!int.pathReady||s.npcs.state[i]!==NPC_STATE.MOVING)continue;const points=s.world.pathfinder.find(s.npcs.x[i],s.npcs.y[i],int.targetX,int.targetY),flat=[];for(const p of points)flat.push(Math.floor(p[0]),Math.floor(p[1]));s.npcs.setRoute(i,Int16Array.from(flat))}
