...
// Primeiro abrigo é primitivo, compartilhável e impede proliferação de canteiros locais.
sim.technology.setKnown(n,i,1,-1,sim.tick);n.inventory[i][RESOURCE.LOG]=20;n.inventory[i][RESOURCE.STONE]=5;
const primitive=w.createBlueprint(BUILDING.SHELTER,n.x[i],n.y[i],n.uid[i]);assert.ok(primitive&&primitive.pieces.length>=4&&primitive.pieces.length<=8,`abrigo primitivo deve ter 4-8 peças, recebeu ${primitive?.pieces?.length}`);primitive.pieces[0].progress=.47;assert.equal(w.nextBlueprintPiece(n.uid[i])?.bp.id,primitive.id,'obra existente deve ser fonte de tarefas');assert.equal(primitive.pieces[0].progress,.47,'progresso precisa permanecer na peça, não no trabalhador');
const ally=n.living().find(j=>j!==i&&Math.hypot(n.homeX[j]-n.homeX[i],n.homeY[j]-n.homeY[i])<=40);assert.ok(ally!=null,'seed de teste precisa conter ao menos um vizinho comunitário');sim.technology.setKnown(n,ally,1,-1,sim.tick);n.inventory[ally][RESOURCE.LOG]=5;assert.equal(w.nextBlueprintPiece(n.uid[ally])?.bp.id,primitive.id,'vizinho deve poder assumir tarefa do abrigo comunitário');assert.equal(w.createBlueprint(BUILDING.SHELTER,n.x[ally],n.y[ally],n.uid[ally]),null,'comunidade sem abrigo não deve abrir um segundo canteiro simultâneo');
const primitiveBuilt=completeBlueprint(sim,primitive);assert.ok(primitiveBuilt?.finished,'abrigo primitivo concluído precisa entrar imediatamente em world.buildings');

// Estruturas consolidadas continuam usando casca completa e cômodos fechados.
const roomX=Math.max(8,Math.min(184,primitive.cx+12)),bp=w.createBlueprint(BUILDING.WORKSHOP,roomX,primitive.cy,n.uid[i]);assert.ok(bp&&bp.pieces.length>20,'edifício consolidado precisa manter projeto estrutural completo');bp.pieces[0].progress=.47;assert.equal(w.nextBlueprintPiece(n.uid[i])?.bp.id,bp.id,'nova obra consolidada deve virar fonte de tarefas');assert.equal(bp.pieces[0].progress,.47,'progresso compartilhado não pode migrar para o trabalhador');
evacuateSite(sim,bp);let built=null;for(const p of bp.pieces.filter(x=>x.layer!=='fixture')){const x=w.completePiece(bp,p);if(x)built=x}assert.ok(built?.finished,'casca concluída precisa produzir estrutura funcional');assert.equal(bp.done,false,'móvel avançado não pode bloquear a estrutura física nem ser confundido com a casca');for(const p of bp.pieces.filter(x=>x.layer==='fixture'))w.completePiece(bp,p);assert.ok(w.pieceStore.filter(Boolean).length>=20,'peças concluídas precisam entrar no store');
...
console.log(`OK Phase G: ${before.pieces} peças, abrigo primitivo, cooperação, cômodos, manutenção, propriedade e território ativos`);